...
class SecondPage(QMainWindow,Form):
    ...
    def SelFile(self):
        ...
        if bool(tempdir):
            ...
            cv2.imwrite("E://Uni Courses & Projects//AP/Final_Project//"+self.ProjDir+ f"modified//{n}.jpg", resized)
            cv2.imwrite("E://Uni Courses & Projects//AP/Final_Project//"+self.ProjDir+ f"//Dataset//{n}.jpg", resized)
            new_file="E://Uni Courses & Projects//AP//Final_Project//"+self.ProjDir+"//modified"
    ...
    def SelFolder(self):
        ...
        if bool(open_file):
            for file in os.listdir(open_file):
                ...
                dim = (width, height)
                resized = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)
                cv2.imwrite("E://Uni Courses & Projects//AP/Final_Project//"+self.ProjDir+ f"//modified//{i}.jpg", resized)
                cv2.imwrite("E://Uni Courses & Projects//AP/Final_Project//"+self.ProjDir+ "//Dataset//%s"% file, resized)
            new_file="E://Uni Courses & Projects//AP//Final_Project//"+self.ProjDir+"//modified"
            for file in os.listdir(new_file):
                pixmap = QPixmap(os.path.join(new_file, file))
                if not pixmap.isNull():
